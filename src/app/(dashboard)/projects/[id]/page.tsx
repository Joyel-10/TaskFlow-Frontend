  ...
  return (
    <div className="p-6 lg:p-8 animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: (project.color || '#8b5cf6') + '22' }}>
        ...
        <div className="flex items-center gap-2">
          <button className="btn-secondary gap-2" onClick={() => setShowAIGenerator(true)}>
            <Bot className="w-4 h-4 text-accent-purple" /> AI Generate
          </button>
          <button className="btn-primary" onClick={() => { setEditTask(null); setDefaultStatus('todo'); setShowTaskModal(true) }}>
        ...
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col}
              ...
              onDrop={e => { e.preventDefault(); if (dragging) handleDrop(dragging, col) }}>
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="p-1 hover:bg-bg-hover rounded text-text-muted hover:text-text-secondary"
                  ...
                  return (
                    <div key={task._id}
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <p className="text-sm font-medium text-text-primary line-clamp-2 flex-1">{task.title}</p>
                        <button className="opacity-0 group-hover:opacity-100 p-1 hover:bg-bg-hover rounded text-red-400 transition-all flex-shrink-0"
                          onClick={e => { e.stopPropagation(); deleteTask(task._id) }}>
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                      {task.description && <p className="text-xs text-text-muted mb-2.5 line-clamp-2">{task.description}</p>}
                      ...
                      {task.aiGenerated && <div className="mt-2 flex items-center gap-1 text-xs text-accent-purple/70"><Bot className="w-3 h-3" /> AI generated</div>}
                    </div>
                  )
                })}
                {colTasks.length === 0 && (
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-text-primary flex items-center gap-2"><Users className="w-4 h-4 text-accent-cyan" /> Team Members ({project.members.length})</h2>
          <button className="btn-secondary text-sm" onClick={() => setShowMemberModal(true)}><Plus className="w-3.5 h-3.5" /> Add Member</button>
        </div>
        <div className="flex flex-wrap gap-3">
          {project.members.map(m => (
            <div key={m.user._id} className="flex items-center gap-2.5 p-2.5 rounded-lg bg-bg-secondary border border-bg-border group">
              ...
              {m.role !== 'admin' && (
                <button className="ml-1 p-1 opacity-0 group-hover:opacity-100 hover:bg-bg-hover rounded text-red-400 transition-all"
      ...
      {showTaskModal && (
        <TaskModal
          ...
          onSaved={(task) => {
            if (editTask) setTasks(prev => prev.map(t => t._id === task._id ? task : t))
      ...
      {showMemberModal && (
        <AddMemberModal
          ...
          onAdded={(updated) => { setProject(updated); setShowMemberModal(false) }}
        />
      )}
      ...
      {showAIGenerator && (
        <AITaskGenerator
          ...
          onTasksCreated={(newTasks) => { setTasks(prev => [...newTasks, ...prev]); setShowAIGenerator(false) }}
        />
      )}
    </div>
  )
