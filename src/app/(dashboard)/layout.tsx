...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setAuthedUser(parsedUser);
      setChecked(true); // ← moved inside try
    } catch {
  ...
  const avatarColor = generateAvatarColor(authedUser.name);
  ...
  return (
    <div className="min-h-screen bg-bg-primary flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-bg-secondary border-r border-bg-border flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
