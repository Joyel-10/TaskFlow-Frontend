...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Directly read from localStorage — no zustand timing issues
        ...
        <div className="p-4 border-t border-bg-border">
          <div className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-bg-hover transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-text-primary truncate">{authedUser.name}</p>
              <p className="text-xs text-text-muted capitalize">{authedUser.role}</p>
            </div>
          </div>
        </div>
      </aside>
